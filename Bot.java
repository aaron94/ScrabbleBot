	...
	private Word findBest(int numTilesInFrame, Board board, Dictionary dictionary) {
		...
		for (int numLetters=1; numLetters<=numTilesInFrame; numLetters++) {
			for (int currPosition=0; currPosition<possiblePositions[numLetters].size(); currPosition++) {
				int startRow = possiblePositions[numLetters].get(currPosition).getStartRow();
				int startColumn = possiblePositions[numLetters].get(currPosition).getStartColumn();
				int direction = possiblePositions[numLetters].get(currPosition).getDirection();
				...
				for (int currAnagram=0; currAnagram<anagrams[numLetters].size(); currAnagram++) {
					...
					if (dictionary.areWords(checkWordInDictionary)) {
						checkWord.setWord(
								possiblePositions[numLetters].get(currPosition).getStartRow(),
								possiblePositions[numLetters].get(currPosition).getStartColumn(),
								possiblePositions[numLetters].get(currPosition).getDirection(),
								possibleWord);
